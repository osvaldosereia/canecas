...
type OrderDraft = { id: string; modelTitle: string; status: string; createdAt: string };
...
function CustomizeSheet({ model, onClose, onApproved }: { model: Model; onClose: () => void; onApproved: (draft: OrderDraft) => void }) {
  const [step, setStep] = useState<"form" | "generating" | "ready" | "error">("form");
  ...
  return <div className="sheet-backdrop" role="presentation"><button className="backdrop-close" onClick={onClose} aria-label="Fechar personalização" /><section className="customize-sheet" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="customize-title" tabIndex={-1}><div className="sheet-handle" /><button className="sheet-close" onClick={onClose} aria-label="Fechar">{icon("close")}</button>
    ...
    {step === "ready" && <div className="ready-state"><span className="ready-badge">Prévia pronta</span><div className="preview-grid"><div className="ready-art">{job?.artImageUrl ? <Image src={job.artImageUrl} alt="Arte personalizada" width={900} height={900} unoptimized /> : <Artwork model={previewModel} />}</div><div className="mug-preview">{job?.mugMockupUrl ? <Image src={job.mugMockupUrl} alt="Mockup da caneca personalizada" width={900} height={900} unoptimized /> : <div className="demo-mug"><span><Artwork model={previewModel} compact /></span></div>}<small>Mockup da caneca</small></div></div><h2>Sua prévia está pronta</h2><p>{job?.mode === "demo" ? "O fluxo seguro já está funcionando em modo de demonstração; basta cadastrar o webhook do Make para receber as imagens reais." : "Confira a arte e o mockup antes de seguir para o pedido."}</p><div className="ready-actions"><button className="secondary-button" onClick={() => setStep("form")}>Ajustar</button><button className="generate-button" onClick={() => job && onApproved({ id: job.id, modelTitle: model.title, status: "Prévia aprovada", createdAt: new Date().toISOString() })}>Aprovar e salvar pedido</button></div></div>}
  </section></div>;
}
  ...
  return <main>
    ...
    <div className="page-shell" id="top">
      ...
      <section className="feed-column">
        ...
        {view === "orders" && (orders.length ? <div className="customer-orders">{orders.map(order => <article key={order.id}><span className="order-icon">✓</span><div><strong>{order.modelTitle}</strong><small>Pedido {order.id.slice(0, 8).toUpperCase()} · {new Intl.DateTimeFormat("pt-BR").format(new Date(order.createdAt))}</small></div><b>{order.status}</b></article>)}</div> : <div className="empty-state"><span>□</span><h2>Nenhum pedido ainda</h2><p>Escolha um modelo, gere sua prévia e aprove para salvá-la aqui.</p><button onClick={() => changeView("discover")}>Escolher um modelo</button></div>)}
      </section>
